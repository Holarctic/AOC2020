...
def extractContained(s):
  if s == "no other bags.":
    return None
  x = s.split(', ')
  y = []
  for i in x:
    y.append((' '.join(i.split(' ')[1:3]), int(i.split(' ')[0])))
    # y.append(' '.join(i.split(' ')[1:3]))
  return y

graph = dict()
for s in input:
  graph[' '.join(s.split(' ')[:2])] = extractContained(' '.join(s.split(' ')[4:]))


# contained = set()
# for abc in range(10):
#   for x, contains in graph.items():
#     if contains is None:
#       continue
#     for i in contains:
#       if i in contained or i == "shiny gold":
#         contained.add(x)
# print(len(contained))


def dfs(bag):
  if graph[bag] is None:
    return 1
  counter = 0
  for torba, kolicina in graph[bag]:
    grana = (dfs(torba)) * kolicina
    counter = counter + grana
  return counter+1
