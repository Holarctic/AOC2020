...
for x in input:
  if len(x) == 8 or (len(x) == 7 and "cid" not in x):

    if 1920<=x["byr"]<=2002 and 2010<=x["iyr"]<=2020 and 2020<=x["eyr"]<=2030:
      if x["ecl"] in eye:
        if re.search(r'^#(?:[0-9a-fA-F]{3}){2}$', x["hcl"]):
          if re.search(r'^\d{9}$', x["pid"]):
            if x["hgt"][-2:] == "cm":
              if 150<=int(x["hgt"][:-2])<=193:
                counter=counter+1
            elif x["hgt"][-2:] == "in":
              if 59<=int(x["hgt"][:-2])<=76:
                counter=counter+1
